...
router.post('/register' ,addLogger, passport.authenticate('register',{session:false, failureRedirect:'/failregister'}), register);
...
router.post('/login',addLogger, passport.authenticate('login',{session: false, failureRedirect:'/faillogin'}), loginJWT);
...
router.get('/current', passportCall('jwt'), applyPolicy(['USER' , 'PREMIUM']), addLogger, (req,res) => {
...
router.get('/github' ,addLogger, passport.authenticate('github',{scope:['user:email']}), async(req,res) =>{
...
router.get('/githubcallback',addLogger, passport.authenticate('github',{session:false, failureRedirect:'/login'}), gitHubCallBack)
